...
// 快捷方式维护 + 共享 profile 一次性迁移（自 main.js 原样迁出，ADR 0002 L2 业务服务层）：
// 修复「没有桌面快捷方式 / 快捷方式指向的文件消失」，让快捷方式图标跟随图标
// 设计更新（.lnk 单独指定 icon.ico）；含 web → web-desktop profile 迁移。
...
const { DESKTOP_PROFILE, desktopProfileDir } = require('./profile');
const { COMPANION_PLUGINS, readJsonFile } = require('./companion-sync');
...
function shortcutIconPath() {
  ...
  try {
    ...
    ctx.log('boot', '复制快捷方式图标失败: ' + err.message);
  ...
  try {
    ...
    for (const duplicate of desktopPlan.removals) {
      try {
        ...
        ctx.log('boot', '清理重复桌面快捷方式失败（已保留）: ' + duplicate + ': ' + err.message);
      }
    }
    ...
    ctx.log('boot', '快捷方式维护失败: ' + err.message);
  }
}
...
function migrateFromSharedWebProfile() {
  try {
    ...
    ctx.log('boot', '共享 profile 迁移失败（不影响启动）: ' + err.message);
  }
}
...
function applyLegacySkinChoice() {
  try {
    ...
    ctx.log('boot', '应用迁移皮肤选择失败: ' + err.message);
  }
}
