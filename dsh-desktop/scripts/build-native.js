...
const MODULES = ['supervisor', 'snapshot'];
...
function sysroot() {
    ...
    if (r.status !== 0)
        throw new Error('rustc 不可用（请先安装 Rust 工具链）');
...
function prepareLldLink() {
    ...
    if (!fs.existsSync(src))
        throw new Error(`rust-lld 不存在: ${src}`);
...
function copyArtifact() {
    ...
    const candidates = process.platform === 'win32' ? [`${artifactBase}.dll`]
