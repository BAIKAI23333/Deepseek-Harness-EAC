...
动态 Cordis 插件（`cordis_define`/`cordis_run`）的定义存在进程内存中，页面重载后需要重新 run，**无法**满足「每打开界面就自动启用」。因此最终采用**静态宿主插件**挂进 Web 组合：

1. **插件文件**：`whale-balance.mjs`（ESM，`export default { name: 'whale-balance-widget', inject: ['webServer', 'credentials'], apply(ctx) {...} }`），放在 Web profile 目录（本机 `C:\Users\Meteor\.dsh\profiles\web\`）。
2. **注册行**：在 profile 的 `cordis.patch.yml`（当前内容为 `[]`）中追加：
   ```yaml
   - insert:
       - id: whale-balance-widget
         name: ./whale-balance.mjs?v=<版本号>
   ```
3. **热更新机制**：profile 的 `cordis.patch.yml` 被 `watchUserPatches` 实时监视，改动即重组合、**无需重启**。但 ESM 会缓存模块——**每次修改插件文件后必须把 `?v=` 版本号 +1**（如 `?v=12`）才能让 loader 重新 import。**必须用 `.mjs`**：`.cjs` 的 require 缓存忽略查询串，实测无法热更新。
4. **宿主上下文**：静态插件运行在宿主进程（非动态沙箱），可直接使用全局 `fetch`（可带自定义请求头）、`node:fs`、`AbortSignal.timeout` 等 Node API。
...
| 路由 | 方法 | 行为 |
|---|---|---|
| `/dsh-whale/image.png` | GET | 读取本地鲸鱼 PNG（主路径 `D:/TestBox/deepseek/DSniang02.png`，回退 `D:/TestBox/deepseek/skin/DSniang02.png`，内存缓存字节），`Content-Type: image/png`、`Cache-Control: public, max-age=3600`；读取失败返回 404。 |
| `/dsh-whale/balance.json` | GET | 返回余额 JSON。**任何情况下都返回 200 + JSON**（`{ok:true,...}` 或 `{ok:false, code, error, transient?}`），绝不悬挂/空响应。 |
| `/dsh-whale/size.json` | GET / PUT | 尺寸持久化：GET 返回 `{scale}` 或 `{}`；PUT 读 body `{scale}` 写盘（主路径 `D:/TestBox/deepseek/.dshw-size.json`，回退工作区 `D:/TestBox/deepseek/skin/.dshw-size.json`），带 CORS 头。 |
...
| 自动刷新 | 60s；变化提示 900ms |
| 尺寸持久化 | `D:/TestBox/deepseek/.dshw-size.json`（回退工作区），scale 0.6–1.4 步进 0.1 |
...
1. 写入 `whale-balance.mjs`，在 `cordis.patch.yml` 追加 insert 行（`.mjs?v=1` 起）。
